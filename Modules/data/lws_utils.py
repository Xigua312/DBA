...
def memonized_get_replacements(word, sememe_dict):
    if word in total_replacements:
        ...
        sememe_tree = sememe_dict.get_sememes_by_word(word, structed=True, merge=False)
        # sememe_tree = sememe_dict.get_sememes_by_word(word, display='list', merge=False, expanded_layer=-1, K=None)
        ...
        for sense in sememe_tree:
            ...
            # synonyms = sense.get('word', {}).get('syn')
            print(sense)
...
#         word_replacements = []
#         # Get candidates using sememe from word
#         sememe_tree = sememe_dict.get_sememes_by_word(word, display='list', merge=False)
#
#         for sense in sememe_tree:
#             # For each sense, look up all the replacements
#             synonyms = sememe_tree['word']['syn']
#             for synonym in synonyms:
#                 actual_word = sememe_dict.get(synonym['id'])[0]['en_word']
#                 actual_pos = sememe_dict.get(synonym['id'])[0]['en_grammar']
#                 word_replacements.append([actual_word, actual_pos])
